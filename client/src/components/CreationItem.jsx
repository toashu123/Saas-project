...
const CreationItem = ({item}) => {
  ...
  return (
    <div onClick={()=> setExpanded(!expanded)} className='p-4 max-w-5xl text-sm bg-white border border-gray-200 rounded-lg cursor-pointer'>
      ...
      <button className='bg-[#EFF6FF] border border-[#BFFBFE] text-[#1E40AF] px-4 py-1 rounded-full '>{item.type} </button>
    </div>
    {
        expanded && (
            <div>
                ...
                <div className='mt-3 h-full overflow-y-scroll text-sm text-slate-700'>
                 {/*the reset-tw will revert the tailwind applied we can see this definition in the index.css file.This is done to make the content looks in structured way.*/} 
